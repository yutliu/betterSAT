...
class ALIVOSTester(TesterBase):
    ...
    extra_hyper_params = dict(device_num=1,
                              data_root="datasets/tianchiyusai",
                              dataset_names=[
                                  "AliVOS1",
                              ],
                              save_video=False,
    ...
    def __init__(self, *args, **kwargs):
        ...
        pipeline: PipelineBase
            pipeline to test
        """
        super(ALIVOSTester, self).__init__(*args, **kwargs)
        self._state['speed'] = -1
    ...
    def run_tracker(self):
        ...
        # setup dataset
        dataset = davis_benchmark.load_dataset(ALI_root, self.dataset_name) # OrderedDict:30 bike-packing, blackswan, bmx-trees...
    ...
    def evaluation(self, search_task_name):
        ...
        hp_dict = {}
        return davis_benchmark.alivos1_eval(davis_data_path,
                                              results_path,
    ...
    def track_single_video_vos(self, tracker, video, mot_enable=True):
        ...
        '''
        if 'DAVIS' in self._hyper_params["data_root"]:
            self.palette = Image.open(self._hyper_params["data_root"] + '/Annotations/480p/'+ video['name'] +'/00000.png').getpalette()

        if 'tianchi' in self._hyper_params["data_root"]:
            self.palette = Image.open(self._hyper_params["data_root"] + '/Annotations/'+ video['name'] +'/00000.png').getpalette()
        ...
        for i in range(pred_mask_final.shape[0]):
            ...
            mask_label.putpalette(self.palette)
            mask_label.save(join(video_path,
