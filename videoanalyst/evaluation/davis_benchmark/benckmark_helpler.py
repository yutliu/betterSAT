...
def load_dataset(davis_path, dataset):
    ...
    #########################
    if 'Ali' in dataset and 'TEST' not in dataset:
        # davis_path = '/data/data_track/DAVIS'
        list_path = join(davis_path, 'ImageSets', 'test1.txt') ###########################
        with open(list_path) as f:
            videos = [v.strip() for v in f.readlines()]

        for video in videos:
            info[video] = {}
            info[video]['anno_files'] = sorted(glob.glob(join(davis_path, 'Annotations', video, '*.png')))
            assert len(info[video]['anno_files']) > 0, logger.error("no anno in path {}".format(join(davis_path, 'Annotations', video)))

            info[video]['image_files'] = sorted(glob.glob(join(davis_path, 'JPEGImages', video, '*.jpg')))
            info[video]['name'] = video
